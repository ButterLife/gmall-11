...
import com.atguigu.admin.ums.vo.UmsAdminParam;
import com.atguigu.admin.utils.JwtTokenUtil;
 ...
 * @CrossOrigin :解决跨域问题
 * 使用jsr303数据校验的三大步骤
 * 1.给需要校验的数据的javaBean上标注校验注解
 * 2.告诉springBoot ,这个数据需要注解
 * springMvn进入方法之前，确定参数的时候会进行校验数据，如果数据不匹配，直接返回错误，不执行方法
 * 3.如何感知校验是否成功
 * 只需要给开启的校验的javabean参数后面，紧跟一个BindingResult对象就可以获取到校验结果
...
public class UmsAdminController {
    ...
    public Object register(@Valid @RequestBody UmsAdminParam umsAdminParam, BindingResult result) throws Exception {
             Admin admin = null;
            //TODO 完成注册功能
       // int i=10/0;
        FileInputStream ssssss = new FileInputStream("ssssss");
        ssssss.read();
